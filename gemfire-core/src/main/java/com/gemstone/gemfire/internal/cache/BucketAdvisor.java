...
public final class BucketAdvisor extends CacheDistributionAdvisor  {
  ...
  // For GemFireXD ALTER TABLE that may change colocation
  ...
  Lock getParentActiveWriteLock() {
    final BucketAdvisor parentAdvisor = this.parentAdvisor;
    if (parentAdvisor != null) {
      return parentAdvisor.getActiveWriteLock();
    }
    return null;
  }
...
//    LogWriter log = getBucket().getCache().getLogger();
...
//    log.fine("getPreferredNode, no available profile");
      return null;
    }
  ...
  public InternalDistributedMember getPreferredInitializedNode(
      final boolean skipUninitializedMembers) {
    final PartitionedRegion pr = getBucket().getPartitionedRegion();
    if (isHosting()) {
      pr.getPrStats().incPreferredReadLocal();
      return getDistributionManager().getId();
    }

    final GemFireCacheImpl cache = pr.getCache();
    InternalDistributedMember member = null;
    final Profile locProfiles[] = this.profiles; // volatile read
    if (locProfiles.length > 0) {
      pr.getPrStats().incPreferredReadRemote();
      // Pick one at random.
      BucketProfile bucketProfile;
      final int randIndex = myRand.nextInt(locProfiles.length);
      for (int index = 0; index < locProfiles.length; index++) {
        // store the last visited in case there is no initialized member
        // after randIndex
        assert locProfiles[index] instanceof BucketProfile;
        bucketProfile = (BucketProfile)locProfiles[index];
        if (bucketProfile.isHosting
            && (!skipUninitializedMembers || !cache
                .isUnInitializedMember(member))) {
          member = bucketProfile.peerMemberId;
        }
        if (member != null && index >= randIndex) {
          break;
        }
      }
    }
  ...
  protected void profileCreated(Profile profile) {
    ...
    if (log.fineEnabled()) {
      log.fine("Profile added " + getBucket().getFullPath()
          + " Profile : " + profile);
    }
    synchronized (this){
  ...
  protected void profileUpdated(Profile profile) {
    ...
    LogWriterI18n log = getLogWriter();
    if (log.fineEnabled()) {
      log.fine("Profile updated " + getBucket().getFullPath()
          + " Profile : " + profile);
    }
    synchronized (this) {
      updateServerBucketProfile();
  ...
  protected void profileRemoved(Profile profile) {
    ...
    LogWriterI18n log = getLogWriter();
    if (log.fineEnabled()) {
      log.fine("Profile removed " + getBucket().getFullPath()
          + " the member lost " + profile.getDistributedMember() + " Profile : " + profile);
    }
    synchronized (this) {
      updateServerBucketProfile();
    }
  ...
  public void checkForLostPrimaryElector(Profile profile) {
    ...
    if(this.primaryElector != null && this.primaryElector.equals(profile.getDistributedMember())) {
      LogWriterI18n log = getLogWriter();
      if(log.fineEnabled()) {
        log.fine("Bucket " + getBucket().getFullPath() + " lost the member responsible for electing the primary. Finishing bucket creation");
      }
  ...
  public boolean putProfile(Profile profile, boolean forceProfile) {
    ...
    if ( ! bp.isHosting && ! bp.isInitializing) {
      if (VERBOSE) {
        getLogWriter().info(
            ...
            "BucketAdvisor#putProfile early out");
      }
    ...
    if (VERBOSE) {
      getLogWriter().info(
          LocalizedStrings.DEBUG,
          "BucketAdvisor#putProfile profile=<" + profile + "> force=" + forceProfile 
          + "; profile = " + bp);
    }
  ...
  public final InternalDistributedMember getPrimary() {
    ...
    if(primary == null){
        primary = waitForNewPrimary(false);
    }
    return primary;
  }
  ...
  public final boolean isPrimaryWithWait() {
    ...
    InternalDistributedMember primary = waitForNewPrimary(true); 
    if(primary != null) {
        return true;
    }
    return false;
   ...
   * This method was split out from getPrimary() due to bug #40639
   * and is only intended to be called from within that method.
   * Now also called from isPrimaryWithWait
   * @param waitForCurrentMember if true, waits for the current member to become primary
   * @see #getPrimary()
   * @return the new primary
   */
  private final InternalDistributedMember waitForNewPrimary(boolean waitForCurrentMember) {
    ...
    timeout += Long.getLong("gemfire.BucketAdvisor.getPrimaryTimeout",
                            15 * 1000);
    InternalDistributedMember newPrimary = waitForPrimaryMember(timeout, waitForCurrentMember);
    return newPrimary;
  }
  ...
  public void removePrimary(InternalDistributedMember member) {
    ...
    try {
      ...
      if (lostPrimary) {
        ...
        BucketListener bl = this.pRegion.getBucketListener();
        if (bl != null) {
          bl.primaryMoved(getBucket());
        }
      }
    }
      ...
      while (!isPrimary()) {
        ...
        if (Thread.currentThread().isInterrupted()) {
          if (logger.fineEnabled()) {
            logger.fine("Breaking from becomePrimary loop due to " +
                "thread interrupt flag being set");
          }
          break;
        }
        if (isClosed() || !isHosting()) {
          if (logger.fineEnabled()) {
            logger.fine("Breaking from becomePrimary loop because " +
                this + " is closed or not hosting");
          }
          break;
        }
        ...
        synchronized (this) {
          if (isVolunteering()) {
            ...
            if (logger.fineEnabled()) {
              logger.fine("Waiting for volunteering thread " + this + 
                  ". Time left: " + waitTime + " ms");
            }
        ...
        if (attemptToDeposePrimary) {
          ...
          if (otherPrimary != null && 
            ...
            if (logger.fineEnabled()) {
              logger.fine("Attempting to depose primary on " +
                  otherPrimary + " for " + this);
            }
            ...
            if (response != null) {
              ...
              if (logger.fineEnabled()) {
                logger.fine("Deposed primary on " + otherPrimary);
              }
            }
          }
          Thread.sleep(10);
  ...
  protected boolean acquiredPrimaryLock() {
    LogWriterI18n log = getLogWriter();
    if (log.infoEnabled()) {
      log.convertToLogWriter().info("Acquired primary lock for BucketID "
          + getBucket().getId() + " PR : "
      ...
      synchronized (this) {
        if (isHosting() && (isVolunteering() || isBecomingPrimary())) {
          ...
          if (requestPrimaryState(IS_PRIMARY_HOSTING)) {
            if (log.fineEnabled()) {
              log
                  .fine("Acquired primary lock for setting primary now BucketID "
                      + getBucket().getId()
                      + " PR : "
                      + regionAdvisor.getPartitionedRegion().getFullPath());
            }
  ...
  protected void acquirePrimaryRecursivelyForColocated() {
    ...
    if (colocatedWithList != null) {
      ...
      for (PartitionedRegion childPR : colocatedWithList) {
        ...
        if (logger.fineEnabled()) {
          logger.fine("BucketAdvisor.acquirePrimaryRecursivelyForColocated"
              + ": about to take lock for bucket: " + getBucket().getId()
              + " of PR  :" + childPR.getFullPath() + " with isHosting="
              + childBA.isHosting());
        }
        ...
        try {
          if (logger.fineEnabled()) {
            logger.fine("BucketAdvisor.acquirePrimaryRecursivelyForColocated"
                + ": Taking lock for bucket: " + getBucket().getId()
                + " of PR  :" + childPR.getFullPath() + " with isHosting="
                + childBA.isHosting());
          }
    ...
    synchronized (this) {
      ...
      try {
        for (;;) {
          ...
          if (primary != null) {
            if (!waitForCurrentMember) {
              return primary;
            } else {
              if (this.regionAdvisor.getDistributionManager().getId().equals(primary)) {
                return primary;
              }
            }
          }
          ...
          if (getLogWriter().fineEnabled()) {
            getLogWriter().fine("Waiting for bucket " + this + ". Time left :" + timeLeft + "ms");
          }
          ...
          if(!loggedWarning) {
            ...
            if(timeUntilWarning <= 0 ) {
              getLogWriter().warning(LocalizedStrings.BucketAdvisor_WAITING_FOR_PRIMARY, new Object[] {warnTime / 1000L, this, this.adviseInitialized()});
              //log a warning;
              loggedWarning=true;
            } else {
        ...
        if(loggedWarning) {
          getLogWriter().info(LocalizedStrings.BucketAdvisor_WAITING_FOR_PRIMARY_DONE);
        }
      }
      return null;
    }
    ...
    synchronized (this) {
      ...
      try {
        for (;;) {
          ...
          if (getLogWriter().fineEnabled()) {
            getLogWriter().fine("Waiting for bucket storage" + this);
          }
          this.wait(timeLeft); // spurious wakeup ok
        }
      }
  ...
  protected void setHosting(boolean value) {
    ...
    final LogWriterI18n logger = this.getLogWriter();
    if (logger.finerEnabled()) {
      logger.finer("setHosting: " + this.toString()
          + " needToVolunteerForPrimary=" + needToVolunteerForPrimary
          + " primaryElector: " + primaryElector);
    }
  ...
  private InternalDistributedMember[] findPrimaryMembers() {
    ...
    if (primaryMembers.size() > 1) {
      if (getLogWriter().fineEnabled()) {
        getLogWriter().fine("[findPrimaryProfiles] " +
            "found the following primary members for " +
...
//            break;
      ...
      case CLOSED:
        switch (requestedState) {
          case CLOSED:
            ...
            this.getLogWriter().warning(LocalizedStrings.BucketAdvisor_ATTEMPTED_TO_CLOSE_BUCKETADVISOR_THAT_IS_ALREADY_CLOSED, e);
            break;
          case VOLUNTEERING_HOSTING:
            // race condition ok, return false to abort volunteering
            return false;
  ...
  /**
   * Returns true if there exists a member with uninitialized bucket i.e. in the
   * process of creation and GII (including self).
   */
  public boolean hasUninitialized() {
    ...
    if (!isHosting() && getProxyBucketRegion()
        .getCreatedBucketRegion() != null) {
      return true;
    }
    return false;
  }
  ...
  class VolunteeringDelegate  {
    ...
    void doVolunteerForPrimary() {
      ...
      try {
        ...
        if (getLogWriter().fineEnabled()) {
          getLogWriter().fine("Begin volunteerForPrimary for " + 
                              BucketAdvisor.this);
        }
        DistributedMemberLock thePrimaryLock = null;
        final BucketAdvisor parentAdvisor = BucketAdvisor.this.parentAdvisor;
        ...
        if (getLogWriter().fineEnabled()) {
          getLogWriter().fine("Exit volunteerForPrimary for " + 
                              BucketAdvisor.this + 
                              "; dlsDestroyed=" + dlsDestroyed);
        }
        endVolunteering();
//        if (isPrimary()) {
...
//          }
//          getLogWriter().severe(LocalizedStrings.DEBUG,
//              "Exit volunteerForPrimary for " + BucketAdvisor.this +
//              " isPrimary()=" + isPrimary() + " bucket=" + bucket);
//        }
      }
    }
    ...
    private void handleException(Exception e, boolean loggit) {
      ...
      if (!safe) {
        if (ENFORCE_SAFE_CLOSE) {
          ...
          getLogWriter().warning(LocalizedStrings.BucketAdvisor_BUCKETADVISOR_WAS_NOT_CLOSED_PROPERLY, e);
        }
      }
    }
    ...
    private void waitIfNoPrimaryMemberFound() {
      synchronized (BucketAdvisor.this) {
        if (basicGetPrimaryMember() == null) {
          waitForPrimaryMember(100, false);
          if (basicGetPrimaryMember() == null ) {
            findAndSetPrimaryMember(); 
          }
        }               
